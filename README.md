...
- CF 가 압축하며 응답 ETag 를 `W/"…"` 로 바꿔 내보내는데, 워커는 R2 `httpEtag`(`"…"`)와 문자열 그대로 비교해 **브라우저 재검증이 늘 200** 이었다(기존 코드부터 있던 결함, 2026-09-26 라이브 실측).
- `if-none-match` 를 `W/` 를 뗀 값으로 비교하고, 목록형(`a, b`)과 `*` 도 받는다.
- 참고: R2 버킷 위치는 **APAC** 인데 한국발 요청이 미국 서부 콜로(SJC·LAX)로 라우팅돼, 캐시 미스 한 건이 4~10초 걸릴 수 있다. ETag 키 무기한 캐싱으로 이 비용은 파일·콜로별 최초 1회만 낸다.
...
2026-09-24 에 넣은 IP 감속의 전 경로 공통 한도(`RL_BURST` 25/10초 → +3초 · `RL_STEADY` 90/60초 → +10초)가
**정상 사용자를 때리고 있었다.** v3 Recs·DBR 첫 진입이 공용 자산 15~19건을 받고 순차 await 단계마다 지연이 쌓여
...
`bands` 선택기가 **정상인 칸을 두 경로로 버리고 있었다.** 실유저 24명 중 2명이 그 때문에
낮은 대역을 받거나 빈 화면이었다. 정본 `nps-reach.md` §7.2-7.

- **표본하한 `100` → `80`**(`BAND_MIN_PLAYED`). §5.5-3b④ 가 실측으로 「80 과 100 은 결과가 동일」을
  확인한 구간의 하단이다. `100` 이면 **81곡짜리 대역이 통째로 표본부족**이 되어 칸이 사라졌다.
- **자 간 단조성(`peak >= avg`) 위반 칸을 clamp 로 살린다.** `reachNps` 는 위반 시 신뢰도가 낮은 쪽
  (`boundaryTotal` 이 작은 쪽)을 `null` 로 지우는데(§5.2), 격자는 `avg`·`peak` 을 **둘 다** 써야
  칸이 선다. 지워진 쪽을 남은 쪽 값으로 끌어올린다 — 도달선이 **높아지는** 방향이라
  「이미 쉬운 곡이 도전으로 올라오는」 위험은 커지지 않는다.
  🔴 **`reachNps` 전역 동작은 무변경** — 격자 호출 경로에서만 보정한다.

**실측(24명 전수, 수정 전후 대조)**: 변화 2명 · **하락/상실 0명** · 나머지 22명 선택 칸 동일.

| 유저 | 수정 전 | 수정 후 | 경로 |
|---|---|---|---|
| ★6.86 | **빈 화면** | `10/exh` (64%, n=81) | 표본하한 |
| ★13.77 | `10/fc` | **`11/exh`** (31%) | clamp (`peak 11.5 < avg 14.04`) |

남은 빈 화면 2명은 별값 `0.02` 둘로 **cold start** 라 현행 추천도 0곡이다.
...
`data/patterns-dp-1112.json` 하나만 로드하기 때문이다. 실유저 `C200074777849` 에서
`exh`/`aa`/`aaa` 가 **전부 3.83** 이라는 무의미한 바닥값이 나왔다 — 그 유저는 lv11~12 에서 EXH 를
거의 못 깨서 도달선이 최하단 빈으로 붕괴했고, `fallback` 플래그로도 안 잡혔다.

🔴 **해법은 워커에서 계산하지 않는 것이다.** 전 레벨 patterns 를 워커에 얹으면 16.6MB 라 메모리가
못 버티고, 격자까지 넣으면 4,224레코드에서 CPU 10ms 예산을 초과한다(`1102` 전례).
⇒ **덤프 파이프라인에서 미리 계산해 싣는다.**

- **새 자산도 새 배급 경로도 필요 없었다** — `loadPersonaResources()` 가 patterns **3밴드
  (`dp-0810`·`dp-1112`·`dp-rest`)를 이미 전부 로드해 deep-merge** 하고 있었다.
  🔴 `patterns-all-slim` 은 쓰지 않는다(Gist 사본이 갱신되지 않는다는 기존 주석 그대로).
- 조인은 제목 정규화가 아니라 **`textage_song_id` + DP 차트 키**로 한다(동명이곡 충돌 회피).
  이를 위해 `chartsFromGridRows()` 산출물에 `textageSongId` 를 **추가**했다(기존 필드는 불변).
- 🔴 **「값 없음」과 「계산 실패」를 가른다** — 조인 0건은 12값 `null` + `total:0` 으로 **정상 반환**하고,
  자산·모듈 로드 실패는 **예외를 그대로 전파**해 호출부가 이전값 보존을 판단한다.
  이 레포는 fail-open 으로 원본을 지운 전례가 있다.
- `meta` 는 통째로 싣지 않고 `fallback`·`total` 만 추린다(덤프가 유저마다 700KB 다).

**실유저 검증**(`C200074777849`): 조인 **1,347 → 4,163** 레코드, 레벨 **1~12 전부**
(lv1 2곡 · lv10 892곡 · lv12 658곡). `avg exh` 3.83 → **10.05**, `aa` 3.83 → **10.16**,
`peak exh` 9.33 → **17.50**. **exh/aa/aaa 동일값 붕괴가 풀렸고** 단조성(ec 16.02 > hc 13.11 > exh 10.05)도 정상.

⚠️ 아직 **덤프에 싣지는 않았다**(`dump-user.mjs` 배선과 R2 백필은 후속). helper 만 추가한 상태다.

### 2026-09-20 — 워커 허용목록에 `users-list-slim.json` 추가 (산출은 됐는데 못 내주고 있었다)

🔴 **바로 위 항목의 후속 결함이다 — 자산을 만들어 올려도 여전히 404 였다.** 증분 병합은
`users-list-slim 갱신: 총 459 명 | 30956 bytes` 를 남기며 **R2 PUT 에 성공**하고 있었는데,
`cf/src/index.js` 의 `ALLOWED_ROOT` 에 키가 없어 **워커가 객체를 보지도 않고 404** 를 냈다.

- 🔴 **"초록인데 404" 의 정체** — 생산자(Actions)와 배급자(워커)가 다른 레포 파일이 아니라 **다른
  계층**이라, 생산 쪽만 고치면 끝난 것처럼 보인다. **새 루트 키를 만들면 워커 허용목록도 같이 열어라.**
  `user/`·`hist/`·`data/` 는 정규식이라 저절로 열리지만 **루트 키만 명시 집합**이다.
- `?fresh=1` 엣지 우회 대상에도 같이 넣었다 — 본체와 같은 생산자·같은 시점이라 본체만 신선하고
  슬림은 낡는 상태를 만들면 안 된다. 같은 이유로 엣지 TTL 도 본체와 같은 60초(별도 분류 없음).


### 2026-09-20 — `users-list-slim.json` 을 실제로 산출한다 (일주일간 404 였다)

🔴 **사고 — 웹이 찾는 슬림 자산이 프로덕션에 없었다.** `data.iidx.in/users-list-slim.json` 은
**404**(루트·`data/` 양쪽)였고, 오소리웹 v3 의 `users-list.js` 폴백 배열이 **항상** 2번째
`users-list.json`(실측 **77,066 B** gzip)으로 떨어지고 있었다. 정본이 약속한 **72 KB → 약 20 KB** 는
실현된 적이 없다.

- **왜 아무도 몰랐나** — 404 를 폴백이 조용히 흡수하고 `console.warn` 만 남긴다. 어디도 빨개지지 않는다.
  🔴 **「없으면 폴백한다」는 규칙은 「없는 게 맞는지」를 따로 검증하지 않으면 무조건 통과 경로가 된다.**
- **왜 없었나** — 산출 코드는 `ohSorryAdmin/scripts/dump-data-repo.js` 에만 있었고(2026-09-13 도입),
  그것은 **수동 전체 덤프일 때만** 돈다. 그런데 `users-list.json` 의 **현행 생산자는 이 레포의 Actions**
  (`merge-user-into-list` 증분 + `dump-users-list` 1일 1회)다. ⇒ 슬림이 붙을 자리에 안 붙어 있었다.

- **`.github/scripts/users-list-slim.mjs` 신설** — `toSlim(list)` 공유 모듈.
  🔴 **공유로 둔 이유는 이것이 이 계약의 세 번째 중복이기 때문이다** — 같은 구조의 중복(dumpUser 스키마
  이중 구현)이 **persona 37명 유실**을 냈다(2026-08-04 항목). 비배열·빈 배열이면 **throw** 한다
  (빈 목록을 PUT 하면 **R2 가 유일본**이라 전 유저 검색이 되돌릴 수 없이 날아간다).
- **`dump-users-list.mjs`** — full 을 쓴 직후 슬림도 쓴다. `--songs` 모드에서는 **쓰지 않는다**
  (그 분기는 full 을 재생성하지 않으므로, 슬림만 최신인 상태를 만들면 안 된다).
- **`dump-users-list.yml`** — 전체 재생성 분기의 `FILES` 에만 추가. `*/30`(songs 전용)은 그대로.
- **`merge-user-into-list.mjs`** — 검증 통과 직후 슬림도 PUT.
  🔴 **슬림의 원본은 `after`(PUT 뒤 다시 읽은 R2 확정본)이지 내가 만든 `list` 가 아니다.**
  `list` 를 쓰면 그 사이 다른 실행이 넣은 유저를 슬림에서 지운다 — 이 파일이 **253명 중 102명을
  옛 값으로 덮은** 사고를 낸 바로 그 지점이다. ⚠️ 슬림 PUT 은 **재시도 루프에 얹지 않는다**
  (full 은 검증까지 끝났으므로 슬림 실패로 다시 병합하면 안 된다).
  - 슬림 PUT 실패는 `::error::` + `exit 1` 로 **빨갛게 끝낸다.** full 성공은 메시지에 갈라 적는다.
    조용히 삼키면 이번 사고가 그대로 재현된다.

- **실측**(459명 기준, 2026-09-20): 비압축 252,031 → **30,956 B**, gzip **77,066 → 6,683 B(91.3% 감소)**.
  `--no-upload` 로컬 실행으로 증분 경로까지 검증했다(슬림 459명 4키 · 방금 병합한 유저 반영 확인).

### 2026-09-20 — 엣지 캐시 우회 `?fresh=1` 추가 (업로드→웹 반영 60초 단축)

크롤링 후 웹에 반영되기까지 1분이 넘었다. 구간을 갈라 보면 `Actions 큐+잡 20~26초` +
**`엣지 캐시 TTL 60초`** 인데, 뒤쪽 60초는 순수한 대기였다.

🔴 **purge 로는 못 푼다.** [cf/src/index.js](cf/src/index.js) 44~52 행에 이미 적어둔 그대로다 —
Cache API(`caches.default`)는 콜로별이라 `cache.delete` 가 그 데이터센터만 지우고, 존 단위
URL purge 는 Cache API 에 아예 안 먹는다. 전역 purge 가 되는 `ctx.cache.purge()` 는
**Workers Caching 이라는 다른 저장소** 용이다. 그래서 무효화가 아니라 **우회**로 간다 —
R2 원본 직독은 read-after-write 강한 일관성이라 방금 PUT 한 것이 바로 보인다.

**동작**

`?fresh=1` 이 붙으면 `cache.match` 를 건너뛰고 R2 에서 직접 읽는다. 그러고도
**`cache.put` 은 그대로 한다** — 폴링이 콜로 캐시를 갱신하는 부수효과가 생겨, 같은 콜로의
다른 사용자도 60초를 기다리지 않는다. 캐시 키는 지금처럼 **쿼리를 버린 정규 키**다.
`fresh=1` 을 캐시 키에 넣으면 일반 요청이 갱신본을 못 보고 캐시만 파편화된다.

**🔴 허용 키를 `user/` · `hist/` · `users-list.json` 으로 제한했다.**

`lib/` · `data/` · `songs.json` 에 `fresh=1` 이 와도 **조용히 무시하고** 평소 캐시 경로로 보낸다.
거부가 아니라 무시인 것은, 막는 게 목적이 아니라 비용이 목적이기 때문이다 —
`data/ohSorryRating.json` raw 2.15MB 등 **용량의 대부분이 그쪽**이라 우회가 열리면
R2 Class B 읽기와 egress 가 폭증한다. 고회전·소용량 키에만 연다.

`if-none-match` → 304 경로는 그대로 뒀다. 폴링 쪽에서 **「아직 안 바뀜」 신호로 쓴다.**

**⚠️ 소비처 주의** — 브라우저 `max-age` 는 여전히 60초다. 폴링하는 쪽이 같은 URL 을 반복
호출하면 **브라우저 자체 캐시에 막혀 폴링이 무의미해진다.** 호출부는 `cache: 'no-store'` 를
반드시 붙일 것.

**남는 지연** — Actions 큐+잡 20~26초는 그대로다. 이 변경으로 없어지는 건 엣지 60초뿐이고,
다른 콜로에서 보는 남들은 여전히 최대 60초 낡은 것을 본다(그건 Workers Caching 이전이라야 풀린다).
